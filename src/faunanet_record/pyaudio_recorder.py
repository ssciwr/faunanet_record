...
class PyAudioRecorder(RecorderBase):
    """
    PyAudioRecorder Implementation of `RecorderBase` that uses `PyAudio` to record data.
    ...
    stop(): Stop PyAudioRecorder and release held resources of PyAudio.
    ...
    def from_cfg(cls, cfg: dict):
        """
        from_cfg Creates a new `PyAudioRecorder` instance. All arguments not given in the `cfg` argument are filled with default values of the constructor.

        Args:
            cfg (dict): Dictionary, obtained from reading a yaml config file, that contains the keyword arguments to construct the `PyAudioRecorder` from.

        Raises:
            ValueError: When the supplied dictionary does not have an 'output_folder' node.

        Returns:
           PyAudioRecorder : A new instance of the `PyAudioRecorder` class, built with the supplied arguments.
        """
...
register_recorder("pyaudio", PyAudioRecorder)
